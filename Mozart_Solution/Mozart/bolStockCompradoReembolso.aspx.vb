...
Partial Class bolStockCompradoReembolso
    ...
    Private Sub CargaProveedores()
        ...
        da.SelectCommand = New SqlCommand
        da.SelectCommand.Connection = cn
        da.SelectCommand.CommandText = "BOL_StockCompradoReembolsoProv_S"
        da.SelectCommand.CommandType = CommandType.StoredProcedure
        da.SelectCommand.Parameters.Add("@IdReg", SqlDbType.Char, 25).Value = viewstate("IdReg")
        Dim ds As New DataSet
    ...
    Private Sub cmdGrabar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdGrabar.Click
        If txtNroPedido.Text.Trim.Length = 0 Then
            lblMsg.Text = "N° Reembolso es obligatorio"
            Return
        End If

        If txtMontoRemision.Text.Trim.Length = 0 Then
            lblMsg.Text = "N° Monto del Reembolso es dato obligatorio"
            Return
        ElseIf Not IsNumeric(txtMontoRemision.Text) Then
            lblMsg.Text = "N° Monto de Reembolso es dato númerico"
            Return
        End If

        If ddlProveedor.Items.Count = 0 Then
            lblMsg.Text = "Falta seleccionar proveedor para reembolso"
            Return
        End If

        Dim cd As New SqlCommand
        cd.Connection = cn
        cd.CommandText = "BOL_StockCompradoReembolso_U"
        ...
        pa.Value = ""
        cd.Parameters.Add("@NroReembolso", SqlDbType.Char, 15).Value = txtNroPedido.Text
