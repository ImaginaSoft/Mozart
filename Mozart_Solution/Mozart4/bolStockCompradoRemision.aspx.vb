...
Partial Class bolStockCompradoRemision
    ...
    Private Sub cmdGrabar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdGrabar.Click
        If txtNroPedido.Text.Trim.Length = 0 Then
            lblMsg.Text = "N° Pedido es dato obligatorio"
            Return
        ElseIf Not IsNumeric(txtNroPedido.Text) Then
            lblMsg.Text = "N° Pedido es dato númerico"
            Return
        End If

        If txtMontoRemision.Text.Trim.Length = 0 Then
            lblMsg.Text = "N° Monto de Remision es dato obligatorio"
            Return
        ElseIf Not IsNumeric(txtMontoRemision.Text) Then
            lblMsg.Text = "N° Monto de Remision es dato númerico"
            Return
        End If

        Dim wNroPropuesta As Integer
        If ddlVersion.Items.Count = 0 Then
            lblMsg.Text = "Pedido no tiene versiones facturadas"
            Return
        End If
        ...
        cd.Connection = cn
        cd.CommandText = "BOL_StockCompradoRemision_U"
        ...
        pa.Value = ""
        cd.Parameters.Add("@NroPedido", SqlDbType.Int).Value = txtNroPedido.Text
        cd.Parameters.Add("@NroPropuesta", SqlDbType.Int).Value = Mid(ddlVersion.SelectedItem.Value, 1, 2)
        cd.Parameters.Add("@NroVersion", SqlDbType.Int).Value = Mid(ddlVersion.SelectedItem.Value, 3, 2)
        cd.Parameters.Add("@FchRemision", SqlDbType.Char, 8).Value = ObjRutina.fechayyyymmdd(txtFchEmision.Text)
        cd.Parameters.Add("@MontoRemision", SqlDbType.Money).Value = txtMontoRemision.Text
    ...
    Private Sub txtNroPedido_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtNroPedido.TextChanged
        ...
        If IsNumeric(txtNroPedido.Text) Then
            iNroPedido = txtNroPedido.Text
        End If
        LeePedido(iNroPedido)
        ...
        da.SelectCommand = New SqlCommand
        da.SelectCommand.Connection = cn
        da.SelectCommand.CommandText = "BOL_StockCompradoVersion_S"
        da.SelectCommand.CommandType = CommandType.StoredProcedure
    ...
    Private Sub LeePedido(ByVal pNroPedido As Integer)
        lblNomCliente.Text = ""
        Dim cd As New SqlCommand
        cd.Connection = cn
