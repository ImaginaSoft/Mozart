...
Partial Class cpcRegistraReembolso
    ...
    Private Sub CargaAbonos()
        Dim ds As New DataSet
        ds = SqlHelper.ExecuteDataset(cn, CommandType.StoredProcedure, "CPC_AbonosCliente_S", New SqlParameter("@CodCliente", Viewstate("CodCliente")))
    ...
    Private Sub cmdGrabar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdGrabar.Click
        ...
        If NC.Text > 0 Then
            lblMsg.Text = "Cliente tiene cargos pendientes de pago, reembolso procede si los cargos estan liquidados."
            Return
        End If
        ...
        For index As Integer = 0 To dgDocumento.Rows.Count - 1
            ...
            If cb.Checked Then
                If Mid(lblSimbolo.Text, 1, 1) <> Mid(dgDocumento.DataKeys(index).Value, 13, 1) Then
                    lblMsg.Text = "Seleccione pagos con el Tipo de moneda en " & lblSimbolo.Text
                    Return
                End If
            End If
        ...
        For index As Integer = 0 To dgDocumento.Rows.Count - 1
            ...
            If cb.Checked Then
                ...
                cd1.Connection = cn
                cd1.CommandText = "CPC_AbonosconSaldo_I"
                ...
                cd1.Parameters.Add("@NroDocumento", SqlDbType.Int).Value = Mid(dgDocumento.DataKeys(index).Value, 3, 10)
                cd1.Parameters.Add("@Tabla", SqlDbType.Char, 1).Value = Mid(dgDocumento.DataKeys(index).Value, 14, 1)
        ...
        '3) Graba el Documento de Reembolso al Cliente
        ...
        Dim cd As New SqlCommand
        cd.Connection = cn
        cd.CommandText = "CPC_RegistraReembolso_I"
        cd.CommandType = CommandType.StoredProcedure

        Dim pa As New SqlParameter
        pa = cd.Parameters.Add("@MsgTrans", SqlDbType.VarChar, 400)
        ...
        cd.Parameters.Add("@CodAutoriza", SqlDbType.VarChar, 20).Value = ""
    ...
    'Dim wTotal As Double = Convert.ToDouble(DataBinder.Eval(e.Item.DataItem, "Saldo"))
    '        wTotalSum += wTotal
    '    ElseIf e.Item.ItemType = ListItemType.Footer Then
    '        e.Item.Cells(7).Text = "Total: " & String.Format("{0:###,###,###,###.00}", wTotalSum)
    '    End If
    'End Sub


    Private Sub CantNotasCargo()
        Dim cd As New SqlCommand
        Dim dr As SqlDataReader
        cd.Connection = cn
        cd.CommandText = "CPC_CantNCxCliente_S"
        cd.CommandType = CommandType.StoredProcedure
        cd.Parameters.Add("@CodCliente", SqlDbType.Int).Value = ViewState("CodCliente")
        Try
            cn.Open()
            dr = cd.ExecuteReader
            Do While dr.Read()
                lblMensaje.Text = "Cliente tiene " & dr.GetValue(dr.GetOrdinal("Cant")) & " Cargo(s) pendiente de Liq."
                NC.Text = dr.GetValue(dr.GetOrdinal("Cant"))
            Loop
            dr.Close()
        Finally
            cn.Close()
