...
Partial Class VtaVersionServicio
	...
	Private Sub CargaDataOpcional()
		Dim ds As New DataSet
		ds = objVersionDet.CargaServiciosAdicional(ViewState("NroPedido"), ViewState("NroPropuesta"), ViewState("NroVersion"))
		dgServicioAD.DataKeyField = "KeyReg"
		dgServicioAD.DataSource = ds
		dgServicioAD.DataBind()
		Dim nReg As Integer = dgServicioAD.Items.Count

		If ViewState("FlagEdita") = "N" Or ViewState("FlagEdita") = "E" Then
			cmdGrabar.Visible = False
			dgServicioAD.Columns(0).Visible = False
			dgServicioAD.Columns(13).Visible = False
			If ViewState("FlagEdita") = "N" Then
				lblMsg.Text = "La Versión es modelo antiguo, no se puede modificar los Servicios"
			Else
				lblMsg.Text = "La Versión es de otra empresa, no se puede modificar los Servicios"
	...
	Private Sub CargaServicioAdicional(ByVal pNroServicio As Integer)
		Dim ds As New DataSet
		Dim wCodProveedor As Integer = 0
		Dim wCodCiudad As String = " "
		Dim wCodTipoServicio As Integer = 0
	...
	Protected Sub dgServicioAD_SelectedIndexChanged(sender As Object, e As EventArgs) Handles dgServicioAD.SelectedIndexChanged
		' Datos del Servicio
		Dim wCodCiudad As String
		Dim wCodProveedor, wCodTipoServicio As Integer
		Dim wNroServicio, wCodTipoAcomodacion As Integer
